...
import { specs, classColors, CLASS_ORDER, specsByClass } from '@/data/bisData'
...
export default function Home() {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      <style>{`
        .spec-row { transition: background 0.15s; text-decoration: none; display: flex; align-items: center; gap: 12px; padding: 12px 18px; }
      ...
      <header style={{ background: 'linear-gradient(180deg,#0d0e14 0%,#181921 100%)', borderBottom: '1px solid var(--border)', padding: '0 24px', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', height: 56, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ background: 'rgba(200,168,75,0.15)', border: '1px solid rgba(200,168,75,0.3)', color: 'var(--accent)', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 4, letterSpacing: '0.08em' }}>CLASSIC</span>
          <span style={{ marginLeft: 'auto', color: 'var(--text-muted)', fontSize: 12 }}>10 classes · 31 specs · 4 phases</span>
        </div>
      </header>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0d0e14 0%,#1a1422 50%,#111218 100%)', borderBottom: '1px solid var(--border)', padding: '48px 24px 40px', textAlign: 'center' }}>
        <h1 style={{ fontSize: 32, fontWeight: 700, color: 'var(--accent)', marginBottom: 12, textShadow: '0 0 40px rgba(200,168,75,0.3)' }}>
          Cataclysm Classic — Best in Slot
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: 15, maxWidth: 560, margin: '0 auto 24px' }}>
          Pre-Raid, T11, T12 y T13 para todas las specs. Enchants, gemas y reforge incluidos.
        </p>
        <div style={{ display: 'flex', gap: 24, justifyContent: 'center', flexWrap: 'wrap' }}>
          {(['DPS','Tank','Healer'] as const).map(role => (
            <span key={role} style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: 13 }}>
              {ROLE_ICON[role]}
              <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{specs.filter(s => s.role === role).length}</span>
              {role}
            </span>
          ))}
        </div>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px 60px' }}>
        <div style={{ display: 'grid', gap: 20, gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))' }}>
          {CLASS_ORDER.map(cls => {
            const clsSpecs = specsByClass(cls)
            if (!clsSpecs.length) return null
                ...
                {clsSpecs.map((spec, i) => {
                  ...
                  return (
                    <Link key={spec.id} href={`/bis/${spec.id}`} className="spec-row" style={{
                      ...
                      <span style={{ background: rc.bg, border: `1px solid ${rc.border}`, color: rc.text, fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 3, letterSpacing: '0.06em', minWidth: 44, textAlign: 'center' }}>
                        {spec.role}
                      </span>
                      ...
                      <span style={{ marginLeft: 'auto', color: 'var(--text-dim)', fontSize: 12 }}>
                        {spec.phases['pre-bis'].length} slots →
                      </span>
                    </Link>
                  )
                })}
